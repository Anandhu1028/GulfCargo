...
class AddToShipController extends BaseController
{
    ...
    public function addMultipleBookingtoshipSubmit(Request $request) {
        // Check if at least one box_id is provided
        if (!$request->has('box_id') || empty($request->box_id)) {
            toastr()->error('Please select at least one booking.');
            return redirect()->back();
        }
    
        // Explode box_ids into an array
        $boxIds = explode(",", $request->box_id);
    
        // Get the shipment (ship) details
        $getShip = Ships::find($request->ship_id);
    
        if (!$getShip) {
            // If the ship is not found, return with an error message
            toastr()->error('Shipment not found.');
            return redirect()->back();
        }
    
        // Loop through each box ID to process bookings
        foreach($boxIds as $boxId) {
            // Get the box by ID
            $boxes = Boxes::find($boxId);
    
            if (!$boxes) {
                // If the box is not found, log the error and continue to the next box
                Log::warning("Box with ID {$boxId} not found.");
                continue; // Skip to the next box if this one doesn't exist
            }
    
            // Update the box with shipment details
            $boxes->is_shipment = 1;
            $boxes->ship_id = $request->ship_id;
            $boxes->save();
    
            // Create a new entry in BoxesStatuses for the booking
            $bookings = new BoxesStatuses();
            $bookings->status_id = $getShip->shipment_status;
            $bookings->box_id = $boxId;
            $bookings->created_at = now();  // Use the current timestamp
            $bookings->save();
        }
    
        // Show success message after all bookings have been processed
        toastr()->success('Booking Added Successfully to the Shipment.');
    
        // Redirect back to the shipment booking page
